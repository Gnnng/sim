...
int CPU::LW(int addr){
    int row = addr / 4;
    int offset = addr % 4;
    int word0 = memory[row];
    int word1 = memory[row + 1];
...
//        int now = IR[PC >> 2];
        ...
        switch(op){
            ...
            //10 0000 lb
            case 0x20:
                reg[rt] = (memory[row] >> (offset * 8)) & 0xFF;
                break;

            //10 1000 sb
            case 0x28:
                temp = (memory[row] >> (offset * 8)) & 0xFF;
                memory[row] ^= (temp ^ (reg[rt] & 0xFF)) << (offset * 8);
                break;
